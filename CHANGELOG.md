...
through them is that Orion could already put things *on* a page and had far
less to say about the page itself, the file around it, or finding any of it.

### Added
- **Redaction.** Orion could draw a filled black rectangle over a name since
  the first release, and that is not redaction — it is a picture of redaction.
  The name stays in the content stream, where selecting the page, copying it,
  or opening it in any other editor hands it straight back, and the black box
  is what persuades someone it is gone. The **Redact** tool draws the same
  rectangle and then, at save time, deletes every object the box covers from
  the page: the text, the images, the drawings. What ships is a black box with
  nothing behind it.

  Two decisions worth writing down. The removal is resolved from where the box
  has *ended up*, not where it was drawn, so a redaction stays an ordinary
  object — draggable, resizable, undoable — right up to the save. And it takes
  everything it **touches** rather than everything it contains: half a covered
  word is not a redaction, and of the two ways to be wrong, removing too much
  is visible on the page and undoable while removing too little is neither.

  It removes what is on the page. The document's metadata is a separate
  question, which is the next entry.
- **Document properties.** Orion has carried a file's title, author, subject
  and keywords across every save since the beginning and never shown them, so a
  document could go out with the previous author's name inside it and nothing
  in the interface would ever have said so. **File ▸ Document Properties** now
  shows the four fields a reader shows and lets you change them — emptying one
  removes it rather than writing a blank — and lists everything else the file
  carries below, read-only, so at least it is visible.
- **Watermarks and page numbers.** Both were possible before and both meant
  placing the same text box once per page. **Tools ▸ Watermark** stamps a word
  across a range with a size, colour, opacity and angle; **Tools ▸ Page
  Numbers** numbers a range from a `Page {n} of {total}` template, in any of
  six positions, starting at whatever number the first numbered page should
  carry.

  A watermark too wide for its page is shrunk to fit, measured along the
  diagonal it will actually sit on, so a long word turned 45° keeps more of its
  size than the same word lying flat. Both write real text rather than a
  picture of it — selectable, searchable — and both produce ordinary objects,
  so a page number that lands on an existing footer can be dragged aside or
  deleted on that one page, and the whole run is a single undo. That last part
  is why they are built out of `TextObject` and needed no new code in the
  writer at all.
- **Export pages as images.** PNG or JPEG, 72 to 600 DPI, whichever pages you
  choose. Rendered from the document as it *would be saved* rather than from
  the file on disk, so unsaved work is in the picture — which is the only
  behaviour that is not a trap, given that the reason to export a page as an
  image is usually to show somebody what you just did to it.
- **A command palette.** `Ctrl/Cmd+Shift+P`, type a few letters, press Enter.
  Orion has some sixty actions across six menus, and the honest failure of a
  menu bar is that it only helps someone who already knows which menu a thing
  is in: watermarks are under Tools, exporting images is under File, and
  neither is guessable. Matching is by letters in order rather than substring,
  so `wm` finds Watermark and `epg` finds Export Pages, and commands that
  cannot run right now are left out rather than greyed — a list that offers
  only what will work.

### Changed
- The screenshots in the README are now generated by `tools/make_screenshots.py`
  rather than taken by hand. Pictures of an interface go stale the moment it
  moves and nobody notices, because looking at them is nobody's routine; and a
  screenshot taken by hand shows whichever document happened to be open, which
  is usually somebody's real work. The script builds its own document, puts one
  of everything on it, and grabs both themes offscreen, so the same command a
...
version number: the tag was re-cut as the work continued rather than advanced,
so what one of those downloads contains is told apart by the date above rather
than by the number. From 1.5.0 the number moves instead.
